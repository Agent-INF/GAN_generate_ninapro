...
def conv_cond_concat(x, y):
    ...
    return concat(
        [x,
         y * tf.ones([x_shapes[0], x_shapes[1], x_shapes[2], y_shapes[3]])], 3)
...
def conv2d(input_,
    ...
    with tf.variable_scope(name):
        ...
        conv = tf.nn.conv2d(
            input_, w, strides=[1, d_h, d_w, 1], padding=padding)
...
def deconv2d(input_,
    ...
    with tf.variable_scope(name):
        ...
        deconv = tf.nn.conv2d_transpose(
            input_, w, output_shape=output_shape, strides=[1, d_h, d_w, 1], padding=padding)
