...
def conv2d(inputs, output_dim, kernel, stride, stddev=0.02, padding="SAME", name="conv2d"):
  """Convolution layer"""
  assert len(
  ...
  with tf.variable_scope(name):
    weights = tf.get_variable(
        'weights', [kernel[0], kernel[1], inputs.get_shape()[-1], output_dim],
...
def deconv2d(inputs, output_shape, kernel, stride, stddev=0.02, padding="SAME", name="deconv2d"):
...
def fully_connect(inputs, output_size, stddev=0.02, bias_init=0.0, name="fully_connect"):
  """Fully-connected layer"""
  shape = inputs.get_shape().as_list()

  with tf.variable_scope(name):
    matrix = tf.get_variable("weights", [shape[1], output_size],
